import { BASE_URL, URL_WEB } from '@/configs';
            ...
            if ((status === 401 || status === 403) && !originalRequest._retry) {
                ...
                if (typeof window !== 'undefined') {
                    ...
                    if (!window.location.pathname.startsWith('/login')) {
                        window.location.href = location.protocol.concat('//').concat(window.location.host) === URL_WEB && env.REACT_APP_ENV !== ENVIRONMENT.DEV ? '/v4' + '/login' : '/login';
                    }
                }
            }
