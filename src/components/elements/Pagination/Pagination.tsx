...
const Pagination = ({ meta, limit, setPage, setParams }: PaginationProps) => {
    const { page, totalData, totalPage, totalDataOnPage } = meta;
    ...
    return (
        <div className="flex items-center justify-between" data-testid="pagination">
            <div>
                <p className="bodySmall italic text-tertiary-400 font-bold">
                    Menampilkan {page * limit + 1 - limit} - {page * limit - limit + totalDataOnPage} dari {totalData}{' '}
                    data
                </p>
            </div>
            <div className="flex items-center gap-1">
                <button
                    ...
                    disabled={page <= 1}
                    className={cn(
                        'border-2 rounded-[4px] p-2 ml-2 bg-white',
                        page >= totalPage ? 'border-tertiary-300' : 'border-tertiary-100'
                    )}
                >
                    <Icon
                ...
                <button
                    ...
                    className={cn(
                        'border-2 rounded-[4px] p-2 ml-2 bg-white',
                        page >= totalPage ? 'border-tertiary-100' : 'border-tertiary-300'
                    )}
                >
                    <Icon
                        className={cn(
